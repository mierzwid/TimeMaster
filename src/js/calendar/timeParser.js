...
const createTimeRangeFromTimeArray = function (startEndTimeArray) {
  ...
  let range = handleLackOfTimePeriodSpecifiedForStartHour(
      startEndTimeArray[0].toLowerCase(),
      startEndTimeArray[1].toLowerCase());

  return new TimeRange({
    startTime: convertToDate(range.start),
    endTime: convertToDate(range.end)
  });
};

const handleLackOfTimePeriodSpecifiedForStartHour = function (start, end) {
  if (start.includes('am') || start.includes('pm')) {
    return {start: start, end: end};
  }
  if (end.includes('am')) {
    return {start: start + 'am', end: end};
  }
  if (end.includes('pm')) {
    return {start: start + 'pm', end: end};
  }
  return {start: start, end: end};
};
